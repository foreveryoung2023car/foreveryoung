...
function renderOrders(orders){
  ...
  if (getOrderView() === 'list' && orders.length > 0) {
    ...
    el.className = '';
    el.innerHTML = '<div class="overflow-x-auto bg-white rounded-lg border border-slate-200"><table class="w-full text-sm"><thead class="bg-slate-50 text-xs"><tr><th class="p-2 text-left">編號</th><th class="p-2 text-left">姓名</th><th class="p-2 text-left">門市</th><th class="p-2 text-left">體驗日</th><th class="p-2 text-center">人</th><th class="p-2 text-center">加值</th><th class="p-2 text-right">總價</th><th class="p-2 text-right">尾款</th><th class="p-2 text-center">狀態</th><th class="p-2 text-right">動作</th></tr></thead><tbody>' + orders.map(o => {
      ...
      return '<tr class="border-t hover:bg-slate-50"><td class="p-2 font-mono text-xs">' + (o.orderId||'') + '</td><td class="p-2 font-bold">' + (o.name||'—') + (function(){var ph=String(o.phone||'').replace(/\D/g,'');if(!ph)return '';var arr=(typeof allOrders!=='undefined'?allOrders:[]);var cnt=arr.filter(function(x){return String(x.phone||'').replace(/\D/g,'')===ph;}).length;return cnt>=3?' <span class="text-[10px] bg-amber-100 text-amber-700 px-1 rounded">⭐'+cnt+'</span>':(cnt>=2?' <span class="text-[10px] text-slate-500">'+cnt+'訪</span>':'');})() + '</td><td class="p-2">' + (o.storeKey||'—') + '</td><td class="p-2 whitespace-nowrap">' + bdStr + '</td><td class="p-2 text-center">' + (o.adults||o.pax||'—') + '</td><td class="p-2 text-center">' + (hair+photo||'—') + '</td><td class="p-2 text-right font-bold">¥' + total.toLocaleString() + '</td><td class="p-2 text-right ' + (due>0?'text-amber-700 font-bold':'text-slate-400') + '">' + (due>0?'¥'+due.toLocaleString():'—') + '</td><td class="p-2 text-center">' + stBadge + '</td><td class="p-2 text-right whitespace-nowrap"><button onclick="openEdit(\'' + o.orderId + '\')" class="px-2 py-1 bg-[#1A365D] text-white text-xs rounded mr-1">✏️</button>' + (!conf ? '<button onclick="quickConfirm(\'' + o.orderId + '\')" class="px-2 py-1 border-2 border-emerald-400 text-emerald-700 text-xs rounded">✅</button>' : '') + '</td></tr>';
    ...
    return '<div class="order-card '+cls+' '+sel+' p-3 md:p-4" style="margin-bottom:0">'+
      '<div class="flex flex-col items-stretch gap-2 md:gap-3">'+
        '<div class="flex items-start gap-2">'+
          ...
          '<div class="flex-1 min-w-0">'+
            '<div class="flex items-center gap-2 mb-2 flex-wrap">'+
              ...
              daysTag+
            '</div>'+
          '<div class="order-grid-inner text-sm bg-slate-50 p-2.5 md:p-3 rounded-lg mb-2">'+
            ...
            '<div><div class="text-xs text-slate-600 font-semibold">電話</div><div class="font-bold flex items-center gap-1">' + (o.phone? '<a href="tel:'+o.phone+'" class="text-[#1A365D] hover:underline">'+o.phone+'</a><button onclick="event.stopPropagation();navigator.clipboard.writeText(\''+o.phone+'\').then(()=>toast(\'已複製電話\'))" class="text-[10px] px-1 bg-slate-100 hover:bg-slate-200 rounded" title="複製">📋</button>' : '—') + '</div></div>'+
        ...
        '<div class="flex flex-row gap-1.5 flex-shrink-0 w-full">'+
          ...
          (!o.confirmed && !anomaly ? '<button onclick="quickConfirm(\''+(o.orderId||'')+'\')" class="px-3 py-1.5 border-2 border-emerald-400 text-emerald-700 rounded-lg text-sm hover:bg-emerald-50 font-semibold flex-1">✅ 快速確認</button>' : '')+
