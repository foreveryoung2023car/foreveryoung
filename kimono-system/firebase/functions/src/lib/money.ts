export function calculateOrderTotal(input: {
  ...
  const discountRate = input.discountRate > 0 && input.discountRate < 10 ? input.discountRate : 10;
  const discountedKimono = Math.round(input.kimonoPriceJpy * discountRate / 10);
  const couponDiscountJpy = Math.max(0, input.kimonoPriceJpy - discountedKimono);
  ...
  return { discountedKimono, couponDiscountJpy, totalJpy, onsiteDueJpy };
}
